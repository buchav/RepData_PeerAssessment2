...
Economical damage includes property and crop damage. We will estimate total economic damage as a sum of property and crop damage. The damage value is represented in two parts - mantissa and exponenta. Mantissa is stored in PROPDMG and CROPDMG fields. Exponenta is stored in PROPDMGEXP and CROPDMGEXP. Exponenta is coded with character "K"=1e3,"M"=1e6,"B"=1e9. Other code of exponent were not defined in reference document so we ignore that records. Let us convert damage to proper numeric value.
...
relevant.columns<-relevant.columns %>% 
        mutate(NPROPDMG=ifelse(toupper(PROPDMGEXP) %in% names(lookup),
                                       lookup[toupper(PROPDMGEXP)]*PROPDMG,0),
               NCROPDMG=ifelse(toupper(CROPDMGEXP) %in% names(lookup),
                               lookup[toupper(CROPDMGEXP)]*CROPDMG,0),
               TOTALDMG=NPROPDMG+NCROPDMG) %>%
        select (-c(PROPDMGEXP,PROPDMG,CROPDMGEXP,CROPDMG))

```

``` {R answer Q1 EVTYPES}
harm_health<-relevant.columns %>% group_by(BTYPE) %>% 
        summarise(INJURIES=sum(INJURIES),FATALITIES=sum(FATALITIES),
                  DMGPROP=sum(NPROPDMG)/10e6,CROPDMG=sum(NCROPDMG)/10e6,
                  TOTALDMG=sum(TOTALDMG)/10e6) %>%
        mutate(TOTAL_HEALTH=INJURIES+FATALITIES) %>% arrange(-TOTAL_HEALTH)

top10.harm_health<-harm_health[1:10] %>% gather(DAMAGE.TYPE,COUNT,-c(BTYPE,TOTAL_HEALTH,CROPDMG,DMGPROP))
...
ggplot(top10.harm_health,aes(x=BTYPE,y=COUNT,fill=DAMAGE.TYPE))+
        ...
        labs(title="Population health damage in US",x="Event",y="Count")
```
